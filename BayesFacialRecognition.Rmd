...
Concerning the posterior distributions, we observe quite large beta's for both p (not recognised versus recognised branch) and q (correctly named versus recognised but not correctly named branch) - as mentioned, this is as expected, since we assumed that fame would strongly positively influence both recognition and naming.

Regarding the posterior predictive distribution, we also see that the model generally fits the outcomes quit splendidly!

Moving on, we visualise how the model fares not globally, but for individual subjects:
...
```
We see that, while the model does quite well for many of the subjects, it doesn't fully manage to capture subjects that recognise and correctly name a lot of items (outcome 2) - this is particularly evident for subjects 1, 2, 3 and 5. This means that the model fails to some degree. A potential fix might be incorporating more information about the subjects in the model (as a tentative guess, older participants might recognise more celebrities in general compared to younger subjects, for instance).

Now, we plot the posterior predictive distributions for four different tiers of fame of the dataset: either those individuals with the highest total page views (A-tier), until those with the least total page views (D-tier). The breaks are equidistant.
...
ppc_bars_grouped(exp$w_ans, gen_data, group = fame_tiers, freq = FALSE) + 
  ggtitle ("Posterior predictive distributions for different tiers of fame")


```
As we see, the posterior predictive distributions for the A & B-tier group are showing a significantly higher proportion of correctly named items (outcome 2 on the x-axis). This is in line with expectations - the more famous the person presented, the more likely it is that they'll be recognised and correctly named. Generally, the model does quite a good job, and the lower the tier, the lower the proportion of outcomes 2.

Let's plot the posterior predictive distributions for the most famous individual in the dataset (Michael Jackson, with total page views being the maximum possible, 1), a "mid-tier celebrity", in this case Bob Dylan, with a total page views of 0.2. The posterior predictive distribution for all other celebrities is visualised for the sake of comparison.
...
```
As we can see, Michael Jackson is correctly named in the vast majority of cases. For Bob Dylan, the posterior predictive distribution becomes slightly less accurate: the margins only barely contain the actual data for outcomes 1, 2 and 4, and do not cover the actual data for option 3. It is worth noting that this may be due to the fact that items with a lower total page views were, by design, presented less frequently during the experiment, and that there are therefore not as many datapoints for less famous individuals in the dataset.

For Steve Jobs, we also see a mismatch - his total page view score is 0.52, but the rate of recognition and naming (outcome 2) is higher than what the model expects. 

In general, it thus seems like the model is well able to generate outcomes that generally match the true outcome distribution, but that it misfits for cases in the middle. Our current hypothesis for this is that, since the total page views metric is exponentially distributed, the current model misses the mark on the "in-between" celebrities. It's possible that an exponential regression would do a better job at fitting the data.
